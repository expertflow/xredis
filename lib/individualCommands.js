...
function auth_callback1(self, pass, callback) {
    return function (err, res) {
        if (err) {
...
function auth_callback2(self, username, pass, callback) {
    return function (err, res) {
        if (err) {
            if (no_password_is_set.test(err.message)) {
                self.warn('Warning: Redis server does not require a password, but a password was supplied.');
                ...
                debug('Redis still loading, trying to authenticate later');
                setTimeout(function () {
                    self.auth(username, pass, callback);
                }, 100);
                return;
            }
        }
...
function acl_callback(self, callback) {
    return function (err, res) {
        utils.callback_or_emit(self, callback, err, res);
    };
}
...
RedisClient.prototype.auth = RedisClient.prototype.AUTH = function auth(firstArg, secondArg, thirdArg) {
    debug('Sending auth to ' + this.address + ' id ' + this.connection_id);

    // Stash auth for connect and reconnect.

    if (thirdArg == null) {
        this.auth_pass = firstArg;
        var ready = this.ready;
        this.ready = ready || this.offline_queue.length === 0;

        var tmp = this.internal_send_command(new Command('auth', [firstArg], auth_callback1(this, firstArg, secondArg)));

    } else {
        this.auth_username = firstArg;
        this.auth_pass = secondArg;
        var ready = this.ready;
        this.ready = ready || this.offline_queue.length === 0;

        var tmp = this.internal_send_command(new Command('auth', [firstArg, secondArg], auth_callback2(this, firstArg, secondArg, thirdArg)));
...
RedisClient.prototype.acl = RedisClient.prototype.ACL = function acl(prem, callback) {

    return this.internal_send_command(new Command('acl', [...prem], acl_callback(this, callback)));
};


// Only works with batch, not in a transaction
Multi.prototype.auth = Multi.prototype.AUTH = function auth(firstArg, secondArg, thirdArg) {
    debug('Sending auth to ' + this.address + ' id ' + this.connection_id);
    ...
    if (thirdArg == null) {
        this.auth_pass = firstArg;
        this.queue.push(new Command('auth', [firstArg], auth_callback(this._client, secondArg)));

    } else {
        this.auth_username = firstArg;
        this.auth_pass = secondArg;
        this.queue.push(new Command('auth', [firstArg, secondArg], auth_callback(this._client, thirdArg)));
    }
